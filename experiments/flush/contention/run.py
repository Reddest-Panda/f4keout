...
"""
    ...
    def read_data(self, dirname):
        ...
        i = 0
        with open(f"{dirname}/" + self.name, "r") as f:
            for line in f:
                if line.strip() == "|<END>|":
                    i += 1
                    continue
...
def graph(measurements):
    ...
    for idx, measurement in enumerate(measurements):
        ...
        l_mu, _ = stats.norm.fit(lower_match_filtered)
        sames.append(l_mu)
        diff_mu, _ = stats.norm.fit(diff_filtered)
        diffs.append(diff_mu)
        threshold = (l_mu + diff_mu) / 2
        ...
        #ax.axvline(x=l_mu, color='r')
        #ax.axvline(x=h_mu, color='r')
...
for vic in options:
    for att in options:
        ...
        for _ in range(ITS):
            os.system(f"./test {vic} {att} > {data}/{timestamp_data}/{vic}-{att}")
            curr_test.read_data(f'{data}/{timestamp_data}')
...
#! --- Process Data --- !#
sames, diffs, thresholds = graph(measurements)
os.system(f"mv times.png {data}/{timestamp_data}")
with open(f'{data}/{timestamp_data}/thresholds.txt', 'w') as f:
    f.write(f'{thresholds}')
with open(f'{data}/{timestamp_data}/sames.txt', 'w') as f:
    f.write(f'{sames}')
with open(f'{data}/{timestamp_data}/diffs.txt', 'w') as f:
    f.write(f'{diffs}')
...
os.system(f"mv compare.png {data}/{timestamp_data}")

## Print results
#os.system(f"xdg-open {data}/{timestamp_data}/times.png")
#os.system(f"xdg-open {data}/{timestamp_data}/compare.png")
