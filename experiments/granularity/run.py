...
def process(readings):
    return [stats.tmean(remove_outliers(addr)) for addr in readings]
...
def graph_all(data, name="prefetching"):
    ...
    for idx, scenario_data in enumerate(separated_data):
        ...
        ax.scatter(x_axis, scenario_data['cl_avg'], label='CL', color='green', linestyle='dotted')
...
## Collect Data
data = pd.DataFrame(columns=["vic", "att", "diff_avg", "lm_avg", "cl_avg"])
for _ in tqdm(range(ITS)):
    for vic, att in itertools.product(*all_cases): # Full sweep loop
        ...
        avgs = process(readings)
        data = pd.concat([data, pd.DataFrame([[vic, att, avgs[0], avgs[1], avgs[2]]], columns=data.columns)], ignore_index=True) # appending row of data
