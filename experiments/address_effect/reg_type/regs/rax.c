...
static inline __attribute__((always_inline)) void writes(uint64_t *ptr) { //! CHANGE
    asm volatile (
        "mov %%rax, 0x0000(%0)\n\t"
        "mov %%rax, 0x1000(%0)\n\t"	
        "mov %%rax, 0x2000(%0)\n\t"
...
static inline __attribute__((always_inline)) void reads(uint64_t *ptr) { //! CHANGE
	asm volatile (
        "mov 0x0000(%0), %%rax\n\t"
        "mov 0x1000(%0), %%rax\n\t" 
        "mov 0x2000(%0), %%rax\n\t" 
...
void *attacker(void *args) {
	...
	uint64_t start, end;
	uint64_t *ptr_diff = mem + 0xABCD; 				//! CHANGE
	uint64_t *ptr_lower_match = mem + OFFSET + 0xA000; //! CHANGE
...
void *victim(void *args) {
	...
	uint64_t *ptr = mem + OFFSET;						//! CHANGE
	mfence();
	while(1) {
		instruction(ptr);
	}
...
int main(int argc, char **argv[]) {
	...
	mem = (uint64_t *)mmap(NULL, 50 * 4096,				 //! CHANGE?
