...
def graph(data):
    ...
    for idx, scenario_data in enumerate(separated_data):
        ...
        addrs = range(len(scenario_data))
        ax.plot(addrs, scenario_data['diff_avg'].values, label='Diff', color='blue', linestyle='.')
        ax.plot(addrs, scenario_data['lm_avg'], label='4k Aliasing', color='orange', linestyle='.')
...
for vic, att, addr in itertools.product(*all_cases):
    ...
    for _ in range(ITS):
        ...
        diff.append(readings[0])
        lm.append(readings[1])
